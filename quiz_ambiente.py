...
def carregar_ranking():
    # Usa session_state para simular um ranking local — poderia ser persistido em arquivo/DB
...
def iniciar_quiz(dificuldade='Medio'):
    # Prepara perguntas embaralhadas e estado do quiz
    perguntas_idx = list(range(len(QUESTOES)))
    random.shuffle(perguntas_idx)
    # Se dificuldade for alta, mostra menos tempo por pergunta (opcional)
...
if 'iniciado' not in st.session_state or not st.session_state.get('iniciado'):
    ...
    with col_c:
        if st.button("Exibir perguntas (todas)"):
            # Modo alternativo - mostra todas as perguntas sem animação
    ...
    with container.container():
        ...
        with col2:
            ...
            if st.button('✅ Salvar resposta'):
                # Guarda resposta e toca som de positivo/negativo conforme valor
                st.session_state['respostas'].append(resposta)
                # animação sonora via JS
                if mostrar_sons:
                    if resposta >= 4:
                ...
                # transição suave: avanço automático com pequeno delay
                st.session_state['cur_idx'] += 1
                # se acabou, vai para tela de resultado
...
if st.session_state.get('finished'):
    ...
    # Recomendação personalizada com base nas respostas
    ...
    # botão para confete e som de vitória
    ...
    with col2:
        if st.button('🔁 Reiniciar quiz'):
            ...
            st.session_state.pop('cur_idx', None)
            st.session_state['iniciado'] = False
            st.session_state['finished'] = False
            st.experimental_rerun()
    with col3:
        if st.button('💾 Salvar no ranking'):
            salvar_no_ranking(nome or 'Anônimo', total)
            st.success('Salvo! Veja no painel lateral.')

    # Mini cartão de compartilhamento - gera texto pronto para redes
...
# -------------------- RODAPÉ / CONTROLE DE SESSÃO --------------------
st.markdown('<div style="margin-top:20px; text-align:center; color:#2f6b3f;">Desenvolvido como ferramenta educativa — personalize-me para sua escola ou feira de ciências!</div>', unsafe_allow_html=True)

# -------------------- FUNÇÃO DE EXIBIÇÃO RÁPIDA (modo rápido) --------------------
# Mantemos aqui para compatibilidade com o modo alternativo usado acima.
...
# Observações finais (comentário para desenvolvedor):
# - O código usa pequenas funções JS injetadas para sons (WebAudio) e confete.
# - Se desejar som de fundo (música), recomendo hospedar um arquivo .mp3 e usar <audio> HTML com controls ocultos.
